...
const Tooltip = ({
  ...
  const renderTooltip = useCallback(
    (attrs: TippyProps) => (
      <span
        className="z-[999] flex items-center gap-2 rounded-lg border border-neutral-100 bg-white px-2.5 py-1 shadow-xs"
        tabIndex={-1}
      ...
      >
        {title && (
          <span className="font-medium text-neutral-500 text-xs">{title}</span>
        )}
        {shortcut && (
