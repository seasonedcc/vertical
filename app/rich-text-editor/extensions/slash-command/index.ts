...
function normalizeForSearch(str: string): string {
  return (
    str
      .toLowerCase()
      .normalize('NFD')
      // biome-ignore lint/suspicious/noMisleadingCharacterClass: intentional combining character removal
      .replace(/[\u0300-\u036f]/g, '')
  )
}
        ...
        items: ({ query }: { query: string }) => {
          const withFilteredCommands = GROUPS.map((group) => ({
            ...
            commands: group.commands
              .filter((item) => {
                const labelNormalized = normalizeForSearch(item.label).trim()
                const queryNormalized = normalizeForSearch(query).trim()

                if (item.aliases) {
                  const aliases = item.aliases.map((alias) =>
                    normalizeForSearch(alias).trim()
                  )
